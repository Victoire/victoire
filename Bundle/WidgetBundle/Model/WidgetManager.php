...
class WidgetManager
{
    ...
    {
        $widget = $this->widgetHelper->newWidgetInstance($type, $page, $slot);

        $classes = $this->annotationReader->getBusinessClassesForWidget($widget);
        $forms = $this->widgetFormBuilder->renderNewWidgetForms($slot, $page, $widget);
    ...
    public function createWidget($type, $slotId, Page $page, $entity)
    {
        ...
        //create a new widget
        $widget = $this->widgetHelper->newWidgetInstance($type, $page, $slotId);

        $form = $this->widgetFormBuilder->callBuildFormSwitchParameters($widget, $page, $entity);

        $form->handleRequest($request);

        if ($form->isValid()) {
            $page = $this->pageHelper->duplicateTemplatePageIfPageInstance($page);
            ...
            $response = array(
                ...
                "html"    => $this->widgetFormBuilder->renderNewForm($form, $widget, $slotId, $page, $entity)
            );
    ...
    public function editWidget(Request $request, Widget $widget, $entity = null)
    {
        ...
        if ($requestMethod === 'POST') {
            ...
            $forms = $this->widgetFormBuilder->renderNewWidgetForms($widget->getSlot(), $page, $widget);
    ...
    public function deleteWidget(Widget $widget)
    {
        ...
        //create a page for the business entity instance if we are currently display an instance for a business entity template
        $page = $this->pageHelper->duplicateTemplatePageIfPageInstance($widgetPage);
