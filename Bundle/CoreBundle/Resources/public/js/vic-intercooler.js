...
var VicIntercooler = VicIntercooler || (function() {
    ...
    var _MACROS = $vic.map(['v-ic-get-from', 'v-ic-post-to', 'v-ic-put-to', 'v-ic-patch-to', 'v-ic-delete-from',
    ...
    function log(elt, msg, level) {
        ...
        if (level == "ERROR") {
            ...
            var errorUrl = closestAttrValue($vic('body'), 'v-ic-post-errors-to');
            if (errorUrl) {
                $vic.post(errorUrl, {'error': msg});
            }
        }
    }
    ...
    function initScrollHandler() {
        if (_scrollHandler == null) {
            _scrollHandler = function() {
                $vic(getICAttributeSelector("v-ic-trigger-on='scrolled-into-view'")).each(function() {
                    var _this = $vic(this);
            ...
            $vic(window).scroll(_scrollHandler);
        }
    }
    ...
    function getTargetImpl(elt, attibuteName) {
        var closest = $vic(elt).closest(getICAttributeSelector(attibuteName));
    ...
    function processHeaders(elt, xhr) {
        ...
        if (xhr.getResponseHeader("X-VIC-Refresh")) {
            ...
            $vic.each(pathsToRefresh, function(i, str) {
        ...
        if (triggerValue) {
            ...
            if (xhr.getResponseHeader("X-VIC-Trigger-Data")) {
                var triggerArgs = $vic.parseJSON(xhr.getResponseHeader("X-VIC-Trigger-Data"));
                triggerEvent(target, triggerValue, triggerArgs);
            } else {
                if (triggerValue.indexOf("{") >= 0) {
                    $vic.each($vic.parseJSON(triggerValue), function(event, args) {
        ...
        if (localVars) {
            $vic.each($vic.parseJSON(localVars), function(key, val) {
    ...
    function closestAttrValue(elt, attr) {
        var closestElt = $vic(elt).closest(getICAttributeSelector(attr));
    ...
    function handleRemoteRequest(elt, type, url, data, success) {
        ...
        var ajaxSetup = {
            ...
            success: function(data, textStatus, xhr) {
                ...
                triggerEvent($vic(document), "success.v-ic", [elt, data, textStatus, xhr, requestId]);
            ...
            complete: function(xhr, status) {
                ...
                try {
                    if ($vic.contains(document, elt[0])) {
                        triggerEvent(elt, "complete.ic", [elt, data, status, xhr, requestId]);
                    } else {
                        triggerEvent($vic('body'), "complete.ic", [elt, data, status, xhr, requestId]);
                    }
                } catch (e) {
        ...
        triggerEvent($vic(document), "beforeAjaxSend.ic", [ajaxSetup, elt]);
    ...
    function processIncludes(data, str) {
        if ($vic.trim(str).indexOf("{") == 0) {
            var obj = $vic.parseJSON(str);
            $vic.each(obj, function(name, value) {
                data = appendData(data, name, value);
            });
        } else {
            $vic(str).each(function() {
                var obj = $vic(this).serializeArray();
                $vic.each(obj, function(i, input) {
    ...
    function processLocalVars(data, str) {
        $vic(str.split(",")).each(function() {
            var key = $vic.trim(this);
    ...
    function getParametersForElement(verb, elt, triggerOrigin) {
        ...
        $vic(getICAttributeSelector('v-ic-global-include')).each(function() {
            data = processIncludes(data, getICAttribute($vic(this), 'v-ic-global-include'));
        });
    ...
    function fireReadyStuff(elt) {
        ...
        $vic.each(_readyHandlers, function(i, handler) {
            try {
                handler(elt);
            } catch (e) {
    ...
    function processMacros(elt) {
        $vic.each(_MACROS, function(i, macro) {
    ...
    function handleAddClasses(elt) {
        ...
        if (getICAttribute(elt, 'v-ic-add-class')) {
            ...
            for (var i = 0; i < arrayLength; i++) {
                parseAndApplyClass($vic.trim(values[i]), elt, 'addClass');
            }
        }
    }
    ...
    function handleRemoveClasses(elt) {
        ...
        if (getICAttribute(elt, 'v-ic-remove-class')) {
            ...
            var arrayLength = values.length;
            for (var i = 0; i < arrayLength; i++) {
                parseAndApplyClass($vic.trim(values[i]), elt, 'removeClass');
            }
        }
    }
        ...
        if (getICAttribute(elt, 'v-ic-trigger-on')) {
            ...
            if(elt.is('form')) {
                elt.on('click focus', 'input, button, select, textarea', function(e){
                    if($vic(this).is('input[type="submit"], button') && $vic(this).is("[name]")) {
                        elt.data('v-ic-last-clicked-button', {name:$vic(this).attr("name"), value:$vic(this).val()})
                    } else {
            ...
            if (getICAttribute(elt, 'v-ic-trigger-on') == 'load') {
                ...
                initScrollHandler();
                setTimeout(function() {
                    triggerEvent($vic(window), 'scroll');
                ...
                if(triggerOn[0].indexOf("sse:") == 0) {
                    ...
                    var triggerOn = getICAttribute($vic(elt), 'v-ic-trigger-on').split(" ");
                    var event = eventFor(triggerOn[0], $vic(elt));
                    $vic(getTriggeredElement(elt)).on(event, function(e) {
                    ...
                    if(event && (event.indexOf("timeout:") == 0)) {
                        setTimeout(function () {
                            $vic(getTriggeredElement(elt)).trigger(event);
    ...
    function isScrolledIntoView(elem) {
        ...
        var docViewTop = $vic(window).scrollTop();
        var docViewBottom = docViewTop + $vic(window).height();
    ...
    function maybeScrollToTarget(elt, target) {
        if (closestAttrValue(elt, 'v-ic-scroll-to-target') != "false" &&
            ...
            var portalTop = $vic(window).scrollTop();
            ...
            if (currentPosition < portalTop || currentPosition > portalEnd) {
                ...
                $vic('html,body').animate({scrollTop: offset}, 400);
            }
        }
    }
    ...
    function maybeFilter(newContent, filter) {
        ...
        if ($vic.zepto) {
            ...
            asQuery = $vic(newDoc).find('body').contents();
        } else {
            asQuery = $vic($vic.parseHTML(newContent, null, true));
        }
        if (filter) {
    ...
    function fixIDs(contentToSwap) {
        ...
        walkTree(contentToSwap, "[id]").each(function() {
            var originalID = $vic(this).attr("id");
            var fixedID;
            do {
                fixedID = "v-ic-fixed-id-" + uuid();
            } while ($vic("#" + fixedID).length > 0);
            fixedIDs[originalID] = fixedID;
            $vic(this).attr("id", fixedID);
        });
        walkTree(contentToSwap, "label[for]").each(function () {
            var originalID = $vic(this).attr("for");
            $vic(this).attr("for", fixedIDs[originalID] || originalID);
        });
        walkTree(contentToSwap, "*").each(function () {
            $vic.each(this.attributes, function () {
    ...
    function invokeLocalAction(elt, actions, modifier) {
        ...
        $vic.each(actionArr, function(i, actionStr) {
            var actionDef = $vic.trim(actionStr);
        ...
        delay = 0;
        $vic.each(actionsArr, function(i, action) {
            delay += action[0];
    ...
    function newIntercoolerHistory(storage, history, slotLimit, historyVersion) {
        ...
        if (historyConfigHasChanged(historySupportData)) {
            log(getTargetForHistory($vic('body')), "Intercooler History configuration changed, clearing history", "INFO");
            clearHistory();
        }
        ...
        function updateLRUList(url) {
            ...
            var t = getTargetForHistory($vic('body'));
        ...
        function saveHistoryData(restorationData) {
            ...
            try {
                ...
                try {
                    ...
                    log(getTargetForHistory($vic('body')), "Unable to save intercooler history with entire history cleared, is something else eating " +
        ...
        function addPopStateHandler(windowToAdd) {
            if (windowToAdd.onpopstate == null || windowToAdd.onpopstate['v-ic-on-pop-state-handler'] != true) {
                ...
                windowToAdd.onpopstate = function(event) {
                    triggerEvent(getTargetForHistory($vic('body')), 'handle.onpopstate.ic');
                    if (!handleHistoryNavigation(event)) {
                        if (currentOnPopState) {
                            currentOnPopState(event);
                        }
                    }
                    triggerEvent(getTargetForHistory($vic('body')), 'pageLoad.ic');
                };
        ...
        function handleHistoryNavigation(event) {
            ...
            if (data && data['v-ic-id']) {
                ...
                if (historyData) {
                    processICResponse(historyData["content"], getTargetForHistory($vic('body')), true);
                    if (historyData["yOffset"]) {
                        window.scrollTo(0, historyData["yOffset"])
                    }
                    return true;
                } else {
                    $vic.get(currentUrl(), {'v-ic-restore-history': true}, function(data, status) {
                        var newDoc = createDocument(data);
                        var replacementHtml = getTargetForHistory(newDoc).html();
                        processICResponse(replacementHtml, getTargetForHistory($vic('body')), true);
                    });
                }
            }
            return false;
    ...
    try {
        ...
        log($vic('body'), "Could not initialize history", "WARN");
    }
    ...
    if($vic.ajaxTransport) {
        $vic.ajaxTransport("text", function(options, origOptions) {
                if (origOptions.url[0] == "#") {
                    var ltAttr = fixICAttributeName("v-ic-local-");
                    var src = $vic(origOptions.url);
                    var rsphdr = [];
                    var status = 200;
                    var statusText = "OK";
                    src.each(function(i, el) {
                        $vic.each(el.attributes, function(j, attr) {
    ...
    function init() {
        ...
        if($vic.zepto) {
            $vic('body').data('zeptoDataTest', {});
            if(typeof($vic('body').data('zeptoDataTest')) == "string") {
    ...
    return {
        ...
        debug: function() {
            ...
            $vic.getScript(debuggerUrl)
                .fail(function(jqxhr, settings, exception) {
                    log($vic('body'), formatError(exception), "ERROR");
                });
        },
        _internal: {
            init: init,
