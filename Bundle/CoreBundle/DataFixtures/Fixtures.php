...
namespace Victoire\Bundle\CoreBundle\DataFixtures;

use Doctrine\Common\Persistence\ObjectManager;
use Nelmio\Alice\Fixtures as AliceFixtures;
use Nelmio\Alice\ORM\Doctrine;
use Nelmio\Alice\ProcessorInterface;
use Psr\Log\LoggerInterface;

class Fixtures extends AliceFixtures
{
    ...
    public function loadFiles($files, array $options = array())
    {
        ...
        if ($this->container instanceof ObjectManager) {
            $persister = new Doctrine($this->container);
        } else {
    ...
    private static function getLoader($class, array $options)
    {
        ...
        if (!isset(self::$loaders[$loaderKey])) {
            $fqcn = '\Victoire\Bundle\CoreBundle\DataFixtures\Loader\VictoireYamlLoader';

            //Init Yaml Loader (use Victoire one to replace user class if needed)
            $yamlLoader = new $fqcn($options['locale'], $options['providers'], $options['seed']);
            $yamlLoader->userClass = $options['user_class'];
            self::$loaders[$loaderKey] = $yamlLoader;
        }
